...
# Local patch (DESN-SW-011): bounded, allocation-free overlapping move on RP2040.
# The original Python byte loop stalled CAN while IRQs were disabled.
@micropython.viper
def _move_left(buffer, destination: int, source: int, count: int):
    data = ptr8(buffer)
...
class Buffer:
    ...
    def finish_write(self, nbytes):
        ...
        try:
            ...
            if self._n == self._w:
                ...
                _move_left(self._b, self._n, self._w, nbytes)
                self._n += nbytes
    ...
    def finish_read(self, nbytes):
        ...
        try:
            ...
            self._n -= nbytes
            _move_left(self._b, 0, nbytes, self._n)
        finally:
            machine.enable_irq(ist)
